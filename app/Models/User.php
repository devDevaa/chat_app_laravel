...
class User extends Authenticatable
{
    ...
    // user can have many group (many to many)
    ...
    public static function getUsersExpectUser(User $user) {
        ...
        $query = User::select(['users.*', 'messages.message as last_message', 'messages.created_at as last_message_date'])
            ...
            ->when(!$user->is_admin, function ($query) {
                $query->whereNull('user.blocked_at');
            })
            ...
            ->orderByRaw('IFNULL(users.blocked_at, 1), messages.created_at desc, users.name asc')
    ...
    public function toConversationArray() {
        return [
            ...
            'last_message_date' => $this->last_message,
        ];
    }
}
